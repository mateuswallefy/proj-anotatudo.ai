...
export function DashboardStatCard({
  ...
  if (isLoading) {
    return (
      <div className="relative rounded-2xl border-0 bg-card p-4 lg:p-5 shadow-md lg:shadow-lg overflow-hidden">
        <div className="flex items-start justify-between mb-3">
          <Skeleton className="h-10 w-10 lg:h-11 lg:w-11 rounded-xl" />
          <Skeleton className="h-5 w-14 lg:h-6 lg:w-16 rounded-full" />
        </div>
        <Skeleton className="h-3 w-16 lg:h-4 lg:w-20 mb-2" />
        <Skeleton className="h-6 lg:h-7 w-24 lg:w-28" />
      </div>
    );
  }
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative rounded-2xl border-0 bg-card p-4 lg:p-5 overflow-hidden",
        "shadow-md lg:shadow-lg hover:shadow-lg lg:hover:shadow-xl transition-shadow duration-300",
        config.glow
      )}
    >
      ...
      <div className="relative flex items-start justify-between mb-3">
        <div
          className={cn(
            "p-2.5 lg:p-3 rounded-xl bg-gradient-to-br backdrop-blur-sm",
            config.iconBg
          )}
        >
        ...
        <div
          className={cn(
            "flex items-center gap-1 px-2 py-1 rounded-full text-[10px] lg:text-xs font-semibold",
            getVariationColor()
          )}
        >
          {getTrendIcon()}
      ...
      <div className="relative">
        ...
        <p className="text-xl lg:text-2xl font-bold tracking-tight text-foreground tabular-nums">
